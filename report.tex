...
\begin{document}
  ...
  \twocolumn[
    ...
    \title{
      %\protect\\ 
      \hspace{0.3zw}自然言語処理parser試作に関する報告書\addtocounter{footnote}{-1}\thanks{2017年12月1日 仮提出 ; 2017年12月6日 本提出}
  ...
  \section{実験の結果}\label{kekka}
    ...
    \subsection{文脈自由文法の実験}
      ...
      作成したプログラムで``the child runs quickly to the large house''をCYK法により構文解析した結果得られたS式を\fig{fig:result_cyk}に示す．
      \begin{table*}[htb]
        \centering
      ...
      ``I saw a girl with a telescope''を構文解析した結果得られたS式を\fig{fig:result_cyk}に示す．
      \begin{figure*}[htpb]
        \centering
        %\resizebox{0.9\linewidth}{!}{
        \fbox{
  ...
  \section{考察}\label{kousatu}
    ...
    後者の解釈となるS式が得られる（\fig{fig:result_telescope}）．
    \begin{figure*}[htpb]
      \centering
      %\resizebox{0.9\linewidth}{!}{
      \fbox{
    ...
    ``Time flies like an arrow''を構文解析した結果，4つのS式（\fig{fig:result_flies}）が得られた．
    \begin{table}[htb]
      \centering
